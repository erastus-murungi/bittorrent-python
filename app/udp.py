...
class BaseTrackerClient:
    def __init__(self, download_info: To, our_peer_id: bytes):
...
class UDPTrackerClient(BaseTrackerClient):
    def __init__(
        ...
        *,
        ...
        self._key = random.randint(
            0, 2**32 - 1
        )  # TODO: maybe implement the same key in HTTPTrackerClient
    ...
    @staticmethod
    def _check_response(
        response: bytes, expected_transaction_id: bytes, expected_action: ActionType
    ):
        actual_action, actual_transaction_id = struct.unpack_from(
            UDPTrackerClient.RESPONSE_HEADER_FMT, response
        )
        ...
        if actual_action != expected_action:
            raise ValueError(
                "Unexpected action ID (expected {}, got {})".format(
    ...
    async def announce(self, server_port: int, event: EventType):
        ...
        try:
            ...
            request = pack(
                "Q",
                UDPTrackerClient.MAGIC_CONNECTION_ID,
                "I",
            ...
            response = await asyncio.wait_for(
                protocol.recv(), UDPTrackerClient.REQUEST_TIMEOUT
            )
            UDPTrackerClient._check_response(
                response, transaction_id, ActionType.connect
            )
            (connection_id,) = struct.unpack_from(
                "!Q", response, UDPTrackerClient.RESPONSE_HEADER_LEN
            )
            ...
            request = pack(
                ...
                connection_id,
                "I",
                ActionType.announce.value,
                "I",
                transaction_id,
                "20s",
                self._download_info.info_hash,
                "20s",
                self._our_peer_id,
                "Q",
                self._statistics.total_downloaded,
                "Q",
                self._download_info.bytes_left,
                "Q",
                self._statistics.total_uploaded,
                "I",
                event.value,
                "I",
            ...
            UDPTrackerClient._check_response(
                response, transaction_id, ActionType.announce
            )
            fmt = "!3I"
            ...
            compact_peer_list = response[
                UDPTrackerClient.RESPONSE_HEADER_LEN + struct.calcsize(fmt) :
            ]
